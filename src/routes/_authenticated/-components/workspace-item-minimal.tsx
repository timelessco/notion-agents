...
export const WorkspaceItemMinimal = ({
  ...
  const handleCreate = useCallback(
    async (kind: "form" | "database") => {
      setIsCreatingForm(true);
      try {
        const { form: newForm } = createFormLocal(workspace.id, { kind });
        router.navigate({
          ...
          <DropdownMenuContent align="start" className="w-48" sideOffset={4}>
            ...
            <DropdownMenuGroup>
              ...
              >
                ...
                <span className="flex-1 text-left">New database</span>
              </DropdownMenuItem>
...
const WorkspaceFormMinimal = ({
  ...
  const prefix =
    form.kind === "database" && !form.icon ? (
      <DatabaseIcon className="size-4 text-muted-foreground" />
    ) : (
      getFormIcon(label, form.icon, form.customization)
    );
