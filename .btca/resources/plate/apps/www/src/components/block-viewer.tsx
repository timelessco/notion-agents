...
function BlockViewerProvider({
  ...
  item,
  tree,
}: Pick<BlockViewerContext, "dependencies" | "highlightedFiles" | "item" | "tree"> & {
  children: React.ReactNode;
  defaultView?: BlockViewerContext["view"];
}) {
  const [view, setView] = React.useState<BlockViewerContext["view"]>(defaultView);
  ...
  React.useEffect(() => {
    if (
      ...
      const loadFiles = async () => {
        ...
        try {
          const response = await fetch(`/api/registry/${encodeURIComponent(item.name)}`);
  ...
  }, [activeFile, hasError, highlightedFiles, isLoading, isSettled, item.name, view]);
    ...
    <div className="flex w-full items-center gap-2 md:pr-[14px]">
      ...
      >
        <TabsList className="h-7 items-center rounded-md p-0 px-[calc(--spacing(1)-2px)] py-[--spacing(1)]">
          <TabsTrigger className="h-[1.45rem] rounded-sm px-2 text-xs" value="preview">
            Preview
          </TabsTrigger>
      ...
      {block && <Separator orientation="vertical" className="mx-2 hidden h-4 sm:flex" />}
      ...
      <div className="ml-auto flex items-center gap-2">
        {!item.meta?.src && !item.meta?.isPro && item.meta?.registry !== false && (
          <>
            {/* NPX Command Button */}
            <Button
              size="icon"
              ...
              onClick={() => {
                copyToClipboard(`npx shadcn@latest add ${siteConfig.registryUrl}${item.name}`);
              }}
            >
              {isCopied ? <Check /> : <Terminal />}

              {block && <span className="hidden lg:inline">npx shadcn@latest add {item.name}</span>}
            </Button>

            {block && <Separator orientation="vertical" className="mx-2 hidden h-4 lg:flex" />}
          </>
        )}
        ...
        {block && (
          <div className="hidden h-7 items-center gap-1.5 rounded-md border p-[2px] shadow-none lg:flex">
            <ToggleGroup
              ...
              type="single"
            >
              <ToggleGroupItem className="size-[22px] rounded-sm p-0" value="100" title="Desktop">
                <Monitor className="size-3.5" />
              </ToggleGroupItem>
              <ToggleGroupItem className="size-[22px] rounded-sm p-0" value="60" title="Tablet">
                <Tablet className="size-3.5" />
              </ToggleGroupItem>
              <ToggleGroupItem className="size-[22px] rounded-sm p-0" value="30" title="Mobile">
              ...
              >
                <Link
                  href={item.meta?.src?.replace("?iframe=true", "") ?? `/blocks/${item.name}`}
                  target="_blank"
                >
...
function BlockViewerView({ height, preview }: { preview: React.ReactNode; height?: string }) {
...
function BlockViewerCode({ size }: { size?: "default" | "sm" }) {
  const { activeFile, dependencies, highlightedFiles, isLoading } = useBlockViewer();
