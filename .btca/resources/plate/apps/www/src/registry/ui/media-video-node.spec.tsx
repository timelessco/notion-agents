...
mock.module("platejs/react", () => ({
  PlateElement: ({ children }: any) => <div data-testid="plate-element">{children}</div>,
...
mock.module("./resize-handle", () => ({
  ...
  Resizable: ({ children }: any) => <div data-testid="resizable">{children}</div>,
...
describe("VideoElement", () => {
  ...
  it("renders a player for plain video URLs inserted via URL", async () => {
    ...
    const view = render(
      <VideoElement attributes={{}} element={{ children: [{ text: "" }], type: "video" } as any}>
        {null}
      </VideoElement>,
    );
  ...
  it("keeps the youtube embed path for youtube videos", async () => {
    ...
    expect(view.getByTestId("youtube-embed").getAttribute("data-id")).toBe("abc123");
    expect(view.queryByTestId("react-player")).toBeNull();
  });
