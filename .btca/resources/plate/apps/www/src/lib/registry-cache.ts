...
export const getCachedRegistryItem = React.cache(
  async (name: string, prefetch = false) => await getRegistryItem(name, prefetch),
);

export const getCachedFileTree = React.cache((files: { path: string; target?: string }[]) =>
...
export const getCachedDependencies = React.cache((name: string) => getAllDependencies(name));
