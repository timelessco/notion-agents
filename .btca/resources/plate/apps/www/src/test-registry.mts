...
const runCommand = async (
  ...
  try {
    ...
    if (!error.stdout?.toString().trim() && !error.stderr?.toString().trim() && error.message) {
...
const testItemsGroup = async (
  ...
  if (!force && fs.existsSync(itemGroupCacheFile)) {
    ...
    try {
      ...
      const parsedCachedArray: RegistryItem[] = JSON.parse(cachedContent) as RegistryItem[];
        ...
        if (blockRegEntry) {
          ...
          if (kitRegEntry) {
            currentDef = {
              ...
              registryDependencies: (kitRegEntry as any).registryDependencies || [], // Assuming structure
            };
          }
        }
        if (currentDef) {
          ...
          task.output = kleur.yellow(`Warning: ${itemName} not found in registry`);
        }
      }
      ...
      if (parsedCachedArray.length === currentDefinitionsForComparison.length) {
        ...
        for (const currentBlock of currentDefinitionsForComparison) {
          const cachedBlock = parsedCachedArray.find((cb) => cb.name === currentBlock.name);
          if (!cachedBlock) {
            allMatch = false;
            const message = kleur.yellow(`Cache invalid: ${currentBlock.name} missing from cache`);
            task.output = message;
            break;
          }
          ...
          if (
            ...
            const addedDeps = Array.from(currentDeps).filter((d) => !cachedDeps.has(d));
            const removedDeps = Array.from(cachedDeps).filter((d) => !currentDeps.has(d));
            let depDiffMsg = `Cache invalid: ${currentBlock.name} dependencies changed`;
            if (addedDeps.length > 0) depDiffMsg += ` (+${addedDeps.join(", ")})`;
            if (removedDeps.length > 0) depDiffMsg += ` (-${removedDeps.join(", ")})`;
          ...
          if (blockDetailsMatch) {
            const currentRegDeps = new Set(currentBlock.registryDependencies || []);
            const cachedRegDeps = new Set(cachedBlock.registryDependencies || []);
            if (
              currentRegDeps.size !== cachedRegDeps.size ||
              !Array.from(currentRegDeps).every((dep) => cachedRegDeps.has(dep))
            ) {
              blockDetailsMatch = false;
              const addedRegDeps = Array.from(currentRegDeps).filter((d) => !cachedRegDeps.has(d));
              ...
              if (addedRegDeps.length > 0) regDepDiffMsg += ` (+${addedRegDeps.join(", ")})`;
              if (removedRegDeps.length > 0) regDepDiffMsg += ` (-${removedRegDeps.join(", ")})`;
    ...
    if (!cacheIsValid || outdatedOrCorrupted) {
      ...
      if (fs.existsSync(itemGroupCacheFile)) {
        try {
          ...
          const message = kleur.yellow(`Failed to remove cache: ${rimrafError}`);
          task.output = message;
        }
      }
    }
  ...
  try {
    ...
    task.output = pinnedOutput + kleur.dim(`Cloning ${path.basename(baseTemplateSourceDir)}...`);
    ...
    if (itemNames.length > 0) {
      ...
      if (baseBlockNameIfCloned && baseTemplateSourceDir !== TEMPLATE_DIR) {
        itemsForShadcnAdd = itemsForShadcnAdd.filter((name) => name !== baseBlockNameIfCloned);
      }
    ...
    fs.writeFileSync(itemGroupCacheFile, JSON.stringify(definitionsForCacheFile, null, 2));

    if (preserveOutputOnSuccess) {
      task.output = pinnedOutput + kleur.green(`✓ Verified ${groupDirName} (output preserved)`);
    } else {
      // await rimraf(targetDir);
      task.output = pinnedOutput + kleur.green(`✓ Verified ${groupDirName} (temp dir cleaned)`);
    }
    // --- END OF SUCCESS PATH ---
  } catch (error) {
...
const main = async () => {
  ...
  if (!fs.existsSync(BASE_TEST_DIR)) fs.mkdirSync(BASE_TEST_DIR, { recursive: true });
  ...
  if (cliItemNames.length > 0) {
    ...
    const shouldPreserveOutput =
      itemsToInstall.length === 1 && registryBlocks.some((b) => b.name === itemsToInstall[0]);
    ...
    if (!shouldPreserveOutput && itemsToInstall.length > 0) {
      ...
      if (registryBlocks.some((b) => b.name === firstItem)) {
        ...
        const potentialCacheMetaFile = path.join(potentialCacheDir, META_CACHE_FILENAME);
        if (fs.existsSync(potentialCacheDir) && fs.existsSync(potentialCacheMetaFile)) {
    ...
    blocksToTestAsSources.forEach((blockName) => {
      if (!registryBlocks.some((b) => b.name === blockName)) {
        console.warn(kleur.yellow(`Block "${blockName}" not found in registry, skipping`));
        return;
      }
    ...
    kitsToTest.forEach((kitName) => {
      ...
      if (!kitDef) {
        console.warn(kleur.yellow(`Kit "${kitName}" not found in registry, skipping`));
        return;
      }
      ...
      dependentGroupTasks.push({
        ...
        task: (_: any, taskWrapper: ListrTaskWrapper<any, any, any>) => {
          ...
          //   'editor-basic.cache'
          // );
          const editorBasicCacheMetaFile = path.join(editorBasicCacheDir, META_CACHE_FILENAME);
          if (!fs.existsSync(editorBasicCacheDir) || !fs.existsSync(editorBasicCacheMetaFile)) {
            taskWrapper.skip(
              kleur.yellow(`Skipped: editor-basic cache not found for kit ${kitName}`),
            );
    ...
    if (config.groups && Array.isArray(config.groups)) {
      config.groups.forEach((groupEntry) => {
        ...
        if (typeof groupEntry === "string") {
          ...
          console.warn(kleur.yellow(`Invalid group entry in config: ${groupEntry}`));
          return; // Invalid group entry
        }

        // Avoid re-testing single blocks that are already source tests
        if (items.length === 1 && sourceBlockTasks.some((sbt) => sbt.id === items[0])) {
          return;
        }
    ...
    if (sourceBlockTasks.length > 0) {
      ...
      listrPhase1.tasks.forEach((task) => {
        ...
        const nameForReporting = taskDef ? taskDef.id : task.title || "Unnamed Task";
...
main().catch((error) => {
  console.error(kleur.red("✖ An unexpected error occurred in the test script:"));
  console.error(error);
  process.exit(1);
});
