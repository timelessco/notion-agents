...
export function generateStaticParams() {
  return [
    ...
    (value, index, array) =>
      index === array.findIndex((candidate) => candidate.slug.join("/") === value.slug.join("/")),
  );
}
...
export default async function DocPage(props: DocPageProps) {
  ...
  if (!doc) {
    ...
    const [tree, highlightedFiles, componentExamples] = await Promise.all([
      ...
      file.meta?.examples
        ? Promise.all(file.meta.examples.map(async (ex: string) => await getExampleCode(ex)))
        : undefined,
    ]);
...
function getRegistryDocs({
  ...
  const usedBy = registryUI.filter(
    (item) =>
      item.meta && Array.isArray(item.meta.examples) && item.meta.examples.includes(docName),
  );
  ...
  const relatedDocs = [
    ...
    .filter(Boolean)
    .filter((doc, index, self) => index === self.findIndex((d) => d.route === doc.route));
  ...
  const groups = [...(file.meta?.docs || []), ...relatedDocs].reduce(
    ...
    { components: [], docs: [], external: [] } as Record<string, typeof relatedDocs>,
  );

  const sortDocs = (docs: typeof relatedDocs) =>
    docs.sort((a: any, b: any) => getDocTitle(a).localeCompare(getDocTitle(b)));

  return [...sortDocs(groups.docs), ...sortDocs(groups.components), ...sortDocs(groups.external)];
}
