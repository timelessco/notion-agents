...
const filterItems = (items: SidebarNavItem[], filter: string): SidebarNavItem[] => {
...
export function NavItemCard({ category, item }: { category: string; item: SidebarNavItem }) {
  const locale = useLocale();
  const Icon = getDocIcon(item, category) ?? CircleDashedIcon;
  return (
    <Link key={item.href} className="rounded-lg" href={hrefWithLocale(item.href!, locale)}>
      <Card className="h-full bg-muted/30 p-0 transition-shadow duration-200 hover:shadow-md">
        <CardContent className="flex gap-2 p-2">
          ...
          <div className="space-y-0">
            ...
            {item.description && (
              <p className="line-clamp-1 text-muted-foreground text-sm">{item.description}</p>
            )}
          </div>
        </CardContent>
      </Card>
...
export function NavItemsGrid({
  ...
  const filteredItems = showFilter
    ? items
        .map((group) => ({
          ...group,
          items: group.items ? filterItems(group.items, filter.toLowerCase()) : undefined,
        }))
    ...
    <div className="py-6 lg:py-8">
      <div className="mb-6 space-y-4">
        <div className="font-bold">
          <DocBreadcrumb value={category} placeholder="Search" items={docSections} />
        </div>
        {showFilter && (
          <Input
            className={cn(
          ...
          <div key={index}>
            {group.title && (
              <div className="group relative">
                <H3 id={group.title.toLowerCase().replace(/ /g, "-")} className="mb-4 scroll-mt-20">
                  <a
            ...
            <div className="grid grid-cols-1 gap-3 xl:grid-cols-2">
              {group.items &&
                flattenItems(group.items).map((item) => (
                  <NavItemCard key={item.href} category={category} item={item} />
                ))}
            </div>
          </div>
        ))}
