...
<Callout type="info">
  快捷键名称（例如示例中的 `toggle`）非常重要，Plate 用它来定位插件上的匹配方法。它首先查找
  transform 方法，如果没有找到则回退到 API 方法。
</Callout>
...
- _（其他选项）_: 您还可以包含与底层 `@udecode/react-hotkeys` 库的 `useHotkeys` 钩子兼容的其他选项，例如 `enabled`、`enableOnContentEditable` 等，以微调行为。
...
const MyCustomBoldPlugin = BoldPlugin.configure({
  shortcuts: {
    toggle: {
      // 这会重新配置 BoldPlugin 的 'toggle' 快捷键
