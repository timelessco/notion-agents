...
export default async function MyStaticPage() {
  ...
  return (
    <PlateStatic editor={editor} style={{ padding: 16 }} className="my-plate-static-content" />
  );
}
```
...
<Callout type="warning" title="Server Component Compatibility">
  `PlateView` cannot be used in Server Components. If you're passing an editor from a server
  component to a client component, you'll encounter serialization errors. Use `PlateStatic` on the
  server side, or create the editor client-side with `usePlateViewEditor`.
</Callout>
