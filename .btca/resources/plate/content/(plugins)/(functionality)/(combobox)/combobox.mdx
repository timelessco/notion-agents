      ...
      <InlineCombobox element={element} trigger="#">
        ...
        <InlineComboboxContent>
          ...
          {tags.map((tag) => (
            ...
            >
              <span className={`w-3 h-3 rounded-full bg-${tag.color}-500 mr-2`} />#{tag.name}
            </InlineComboboxItem>
          ))}
        </InlineComboboxContent>
      </InlineCombobox>
...
<API name="TriggerComboboxPluginOptions">
  <APIOptions>
    ...
    <APIItem name="trigger" type="RegExp | string[] | string">
      Character(s) that trigger the combobox. Can be: - A single character (e.g. '@') - An array of
      characters - A regular expression
    </APIItem>
    <APIItem name="triggerPreviousCharPattern" type="RegExp" optional>
      Pattern to match the character before trigger. - **Example:** `/^\s?$/` matches start of line
      or space
    </APIItem>
