...
export const commentPlugin = toTPlatePlugin<CommentConfig>(BaseCommentPlugin, ({ editor }) => ({
  options: {
    activeId: null,
...
<KeyTable>
  <KeyTableItem hotkey="Cmd + Shift + M">在选中文本上添加评论。</KeyTableItem>
</KeyTable>
...
<API name="node">
  ...
  <APIReturns type="NodeEntry<TCommentText> | undefined">如果找到则返回评论节点条目。</APIReturns>
</API>
...
<API name="nodes">
  ...
  <APIReturns type="NodeEntry<TCommentText>[]">评论节点条目数组。</APIReturns>
</API>
...
<API name="isCommentNodeById">
  ...
  <APIReturns type="boolean">节点是否是具有指定 ID 的评论。</APIReturns>
</API>
