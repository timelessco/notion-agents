...
function SuggestionToolbar() {
  ...
  return <button onClick={toggleSuggesting}>{isSuggesting ? "停止建议" : "开始建议"}</button>;
}
```
...
<KeyTable>
  <KeyTableItem hotkey="Cmd + Shift + S">在选中文本上添加建议</KeyTableItem>
</KeyTable>
...
<API name="dataList">
  ...
  <APIReturns type="TInlineSuggestionData[]">建议数据数组。</APIReturns>
</API>
...
<API name="isBlockSuggestion">
  ...
  <APIReturns type="node is TSuggestionElement">是否为区块建议。</APIReturns>
</API>
...
<API name="nodeId">
  ...
  <APIReturns type="string | undefined">找到的建议ID。</APIReturns>
</API>
...
<API name="nodes">
  ...
  <APIReturns type="NodeEntry<TElement | TSuggestionText>[]">建议节点条目数组。</APIReturns>
</API>
