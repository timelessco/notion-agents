...
<API name="AIChatPlugin">
  <APIOptions>
    ...
    <APIItem name="mode" type="'chat' | 'insert'">
      Controls whether responses stream directly into the document or open a review panel. Defaults
      to `'insert'`.
    </APIItem>
...
<API name="node">
  <APIParameters>
    <APIItem
      ...
      optional
    >
      Set `anchor: true` to get the anchor node or `streaming: true` to retrieve the node currently
      being streamed into.
    </APIItem>
  </APIParameters>
...
<API name="hide">
  <APIParameters>
    <APIItem name="options" type="{ focus?: boolean; undo?: boolean }" optional>
      Set `focus: false` to keep focus outside the editor or `undo: false` to preserve inserted
      content.
    </APIItem>
  </APIParameters>
</API>
...
<API name="beginPreview">
  <APIParameters>
    <APIItem name="options" type="{ originalBlocks?: Value }" optional>
      Top-level blocks that the preview will overwrite. Use `[]` when preview inserts after existing
      content.
    </APIItem>
  </APIParameters>
  <APIReturns type="boolean">
    Returns `true` when a new preview rollback point was stored, or `false` when preview state
    already exists.
  </APIReturns>
</API>
