...
- **插入和聊天审查模式**，通过局部回滚的插入预览以及 `withAIBatch` 和 `tf.ai.undo()` 实现撤销安全批处理。
- **块选择感知变换**，使用 `tf.aiChat.replaceSelection` 和 `tf.aiChat.insertBelow` 替换或追加整个部分。
- **与 `@ai-sdk/react` 直接集成**，使 `api.aiChat.submit` 可以从 Vercel AI SDK 助手流式传输响应。
...
- [`AILoadingBar`](/docs/components/ai-loading-bar)：在编辑器容器中显示流式状态。
- [`AIAnchorElement`](/docs/components/ai-anchor-element)：用于在流式传输期间定位浮动菜单的不可见锚点节点。
- [`AILeaf`](/docs/components/ai-leaf)：以微妙的样式渲染 AI 标记的文本。
...
使用 `AIPlugin.withComponent` 配合您自己的元素（或 [`AILeaf`](/docs/components/ai-leaf)）来高亮 AI 生成的文本。
...
- `ctx.children` 和 `ctx.selection` 被重新水合到 Slate 编辑器中，以便您可以构建丰富的提示（参见[提示模板](#提示模板)）。
...
- 返回流式响应，以便 `useChat` 和 `useChatChunk` 可以增量处理令牌。
...
- 将其与 `replacePlaceholders(editor, template, { prompt })` 结合使用，以使用 `@platejs/ai` 生成的 Markdown 扩展 `{editor}`、`{block}`、`{blockSelection}` 和 `{prompt}`。
...
- 工具助手如 `getMarkdown`、`getMarkdownWithSelection` 和 `buildStructuredPrompt`（参见 `apps/www/src/app/api/ai/command/prompts.ts`）使将块 ID、选择和 MDX 标签嵌入 LLM 请求变得容易。
...
- `streamDeserializeMd` 和 `streamDeserializeInlineMd` 提供更低级别的访问，如果您需要控制自定义节点类型的流式传输。
...
<API name="useEditorChat">
  <APIParameters>
    <APIItem name="onOpenBlockSelection" type="(blocks: NodeEntry[]) => void" optional>
      在块选择上打开菜单时调用。
    </APIItem>
    <APIItem name="onOpenChange" type="(open: boolean) => void" optional>
      菜单打开或关闭时调用。
    </APIItem>
    <APIItem name="onOpenCursor" type="() => void" optional>
      在光标处打开菜单时调用。
    </APIItem>
    <APIItem name="onOpenSelection" type="() => void" optional>
      在文本选择上打开菜单时调用。
    </APIItem>
  </APIParameters>
</API>
...
<API name="useChatChunk">
  <APIParameters>
    <APIItem
      name="onChunk"
      type="(chunk: { chunk: string; isFirst: boolean; nodes: TText[]; text: string }) => void"
    >
      处理每个流式块。
    </APIItem>
    <APIItem name="onFinish" type="({ content }: { content: string }) => void" optional>
      流式传输完成时调用。
    </APIItem>
  </APIParameters>
</API>
...
<API name="withAIBatch">
  <APIParameters>
    ...
    <APIItem name="options" type="{ split?: boolean }" optional>
...
<API name="aiCommentToRange">
  <APIParameters>
    ...
    <APIItem name="options" type="{ blockId: string; comment: string; content: string }">
      用于定位范围的块 ID 和文本。
    </APIItem>
  </APIParameters>
...
<API name="getEditorPrompt">
  <APIParameters>
    ...
    <APIItem name="options" type="{ prompt?: EditorPrompt }">
      描述提示的字符串、配置或函数。
    </APIItem>
  </APIParameters>
...
<API name="replacePlaceholders">
  <APIParameters>
    ...
    <APIItem name="options" type="{ prompt?: string }" optional>
      注入到 `{prompt}` 中的提示值。
    </APIItem>
  </APIParameters>
...
<API name="AIPlugin">
  <APIOptions>
    <APIItem name="node.isLeaf" type="true">
      AI 内容存储在文本节点上。
    </APIItem>
    <APIItem name="node.isDecoration" type="false">
      AI 标记是常规文本属性，而不是装饰。
    </APIItem>
  </APIOptions>
</API>
...
<API name="AIChatPlugin">
  <APIOptions>
    ...
    <APIItem name="chat" type="UseChatHelpers&lt;ChatMessage&gt;" optional>
      存储来自 `useChat` 的助手，以便 API 调用可以访问它们。
    </APIItem>
    <APIItem name="chatNodes" type="TIdElement[]" optional>
      用于差异编辑建议的节点快照（内部管理）。
    </APIItem>
    <APIItem name="chatSelection" type="TRange | null" optional>
      提交提示前捕获的选择（内部管理）。
    </APIItem>
    <APIItem name="mode" type="'chat' | 'insert'">
    ...
    <APIItem name="toolName" type="'comment' | 'edit' | 'generate' | null" optional>
      用于解释响应的活动工具。
    </APIItem>
  </APIOptions>
</API>
...
<API name="submit">
  <APIParameters>
    <APIItem name="input" type="string">
      来自用户的原始输入。
    </APIItem>
  ...
  <APIOptions type="object">
    ...
    <APIItem name="options" type="ChatRequestOptions" optional>
      转发到 `chat.sendMessage`（model、headers 等）。
    </APIItem>
    <APIItem name="prompt" type="EditorPrompt" optional>
      由 `getEditorPrompt` 处理的字符串、配置或函数。
    </APIItem>
    <APIItem name="toolName" type="'comment' | 'edit' | 'generate' | null" optional>
...
<API name="reset">
  <APIParameters>
    <APIItem name="options" type="{ undo?: boolean }" optional>
...
<API name="node">
  <APIParameters>
    <APIItem
      name="options"
      type="EditorNodesOptions &amp; { anchor?: boolean; streaming?: boolean }"
      optional
    >
...
<API name="hide">
  <APIParameters>
    <APIItem name="options" type="{ focus?: boolean; undo?: boolean }" optional>
...
<API name="insertBelow">
  <APIParameters>
    <APIItem name="sourceEditor" type="SlateEditor">
      包含生成内容的编辑器。
    </APIItem>
    <APIItem name="options" type="{ format?: 'all' | 'none' | 'single' }" optional>
      从源选择复制格式。默认为 `'single'`。
    </APIItem>
  </APIParameters>
</API>
...
<API name="beginPreview">
  <APIParameters>
    <APIItem name="options" type="{ originalBlocks?: Value }" optional>
...
<API name="acceptPreview">
  <APIReturns type="boolean">当活动预览被提交时返回 `true`。</APIReturns>
</API>
...
<API name="cancelPreview">
  <APIReturns type="boolean">当活动预览被恢复时返回 `true`。</APIReturns>
</API>
...
<API name="discardPreview">
  <APIReturns type="boolean">当活动预览状态被清除时返回 `true`。</APIReturns>
</API>
...
<API name="hasPreview">
  <APIReturns type="boolean">当存在预览回滚状态时返回 `true`。</APIReturns>
</API>
