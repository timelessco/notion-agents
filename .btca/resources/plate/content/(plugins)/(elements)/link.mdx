...
<APIReturns type="boolean">Returns `true` if the link was inserted successfully.</APIReturns>
...
<API name="CreateLinkNodeOptions">
  <APIOptions type="object">
    ...
    <APIItem name="text" type="string" optional>
      The text that is displayed for the link node. If not provided, the URL is used as the display
      text.
    </APIItem>
    <APIItem name="target" type="string" optional>
      Specifies where to open the URL: - `_blank`: new tab - `_self`: same frame - `_parent`: parent
      frame - `_top`: full window
    </APIItem>
