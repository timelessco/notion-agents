...
<API name="TablePlugin">
  <APIOptions>
    ...
    <APIItem name="enableUnsetSingleColSize" type="boolean" optional>
      Disables unsetting the width of the first column when the table has only one column. Set this
      to `true` if you want to resize the table width when there's only one column. Leave it `false`
      if you have a full-width table.
    </APIItem>
    <APIItem name="initialTableWidth" type="number" optional>
      If defined, a normalizer will set each undefined table `colSizes` to this value divided by the
      number of columns. Note that merged cells are not yet supported.
    </APIItem>
...
<API name="insert.tableRow">
  <APIOptions>
    <APIItem name="at" type="Path" optional>
      Exact path of the row to insert the column at. Pass the table path to insert at the end of the
      table. Will overrule `fromRow`.
    </APIItem>
    <APIItem name="before" type="boolean" optional>
...
<API name="moveSelectionFromCell">
  <APIOptions>
    ...
    <APIItem name="reverse" type="boolean" optional>
      Set to `true` to move the selection to the cell above, `false` to move the selection to the
      cell below.
    </APIItem>
