...
<Callout type="warning" title="前置条件">
  开始之前，请确保您已安装并配置好 [shadcn/ui](https://ui.shadcn.com/docs/installation/next) 和
...
<Callout type="info">
  `usePlateEditor` 创建一个记忆化的编辑器实例，确保在重新渲染时保持稳定。如需非记忆化版本，请使用
  `createPlateEditor`。
</Callout>
...
<Callout type="info">
  `basic-nodes-kit`
  包含所有基础插件（粗体、斜体、下划线、标题、引用等）及其组件，我们将在后续步骤中使用。
</Callout>
