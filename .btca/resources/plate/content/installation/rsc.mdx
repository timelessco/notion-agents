...
<Callout type="info">
  `createSlateEditor` creates a raw Plate editor instance. It's suitable for server-side logic, data
  transformation, or preparing content for static rendering. Check out the full [Server-Side
  Example](/docs/examples/server-side) to see it in action.
</Callout>
