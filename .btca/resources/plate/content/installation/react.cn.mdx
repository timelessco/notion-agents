...
<Callout type="warning" title="先决条件">
  开始前，请确保已安装并配置 [shadcn/ui](https://ui.shadcn.com/docs/installation)（适配您的框架，如
  Vite）和 [Plate UI](/docs/installation/plate-ui)。
</Callout>
...
<Callout type="info">
  `usePlateEditor` 创建了一个记忆化的编辑器实例，确保在重新渲染时保持稳定。如需非记忆化版本，请使用
  `createPlateEditor`。
</Callout>
