...
import { HotkeysProvider, useHotkeysContext, type HotkeysContextType } from "./HotkeysProvider";

const createWrapper =
  (initiallyActiveScopes?: string[]) =>
  ({ children }: { children: ReactNode }) => (
    <HotkeysProvider initiallyActiveScopes={initiallyActiveScopes}>{children}</HotkeysProvider>
  );
...
describe("HotkeysProvider", () => {
  ...
  it("drops bound hotkeys when their scopes stop matching", () => {
    const { rerender, result } = renderHook<HotkeysContextType, { scopes: string[] }>(
      ({ scopes }) => {
