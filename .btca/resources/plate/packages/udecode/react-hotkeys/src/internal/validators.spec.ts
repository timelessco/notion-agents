...
describe("validators", () => {
  ...
  it("checks enabled state and tag allowlists", () => {
    ...
    expect(isHotkeyEnabled(inputEvent, { keys: ["a"] } as any, () => false)).toBe(false);
  ...
  it("matches keyboard events through produced keys, modifiers, and pressed-key fallback", () => {
    expect(
      isHotkeyMatchingKeyboardEvent(createKeyboardEvent({ code: "Slash", key: "?" }), {
        alt: false,
        ctrl: false,
        keys: ["?"],
        meta: false,
    ...
    expect(
      isHotkeyMatchingKeyboardEvent(createKeyboardEvent({ code: "KeyK", ctrlKey: true }), {
        alt: false,
        ctrl: false,
        keys: ["k"],
        meta: false,
