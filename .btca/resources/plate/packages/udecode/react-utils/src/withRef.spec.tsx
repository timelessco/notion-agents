...
describe("withRef", () => {
  it("forwards refs to the rendered element", () => {
    const Button = withRef<"button", { label: string }>(({ label, ...props }, ref) => (
    ...
    const { getByTestId } = render(<Button data-testid="button" label="Click" ref={ref} />);
  ...
  it("passes props through to the rendered element", () => {
    ...
    expect((getByRole("button", { name: "Click" }) as HTMLButtonElement).disabled).toBe(true);
