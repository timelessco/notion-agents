...
async function fetchPackageVersion(
  ...
  try {
    ...
    const { stdout } = await execPromise(`npm view ${versionSpecifier} version --json`);
    const versions = JSON.parse(stdout);
    const latestMatchingVersion = Array.isArray(versions) ? versions.at(-1) : versions;
...
async function fetchPackageVersions(
  ...
  const versionPromises = packagesToFetch.map(async (pkg) => {
    ...
    if (version) {
      const currentVersion =
        currentPackageJson.dependencies?.[pkg]?.replace(VERSION_PREFIX_REGEX, "") ||
        currentPackageJson.devDependencies?.[pkg]?.replace(VERSION_PREFIX_REGEX, "") ||
        "Not installed";
  ...
  const versionMap = new Map<string, { currentVersion: string; version: string }>(
...
async function preparePackageUpdates(
  ...
  for (const [name, versions] of Array.from(versionMap.entries())) {
    ...
    if (
      ...
      newPackageJson.dependencies[name].replace(VERSION_PREFIX_REGEX, "") !== versions.version
    ) {
    ...
    if (
      ...
      newPackageJson.devDependencies[name].replace(VERSION_PREFIX_REGEX, "") !== versions.version
    ) {
...
async function runSync(options: DepSyncOptions) {
  ...
  const versionMap = await fetchPackageVersions(packagesToFetch, currentPackageJson, options);
  ...
  if (updatedPackages.length > 0) {
    ...
    if (proceed) {
      ...
      if (shouldRunInstall) {
        ...
        const installSpinner = createSpinner(`Running \`${installCommand}\`...`, {
          silent: options.silent,
        })?.start();
        try {
...
async function main() {
  ...
  program
    ...
    .version(packageJson.version || "0.1.0", "-v, --version", "display the version number")
    .argument(
      "[package-specifier]",
      ...
      try {
        ...
        if (!pkgSpec && !cliOpts.yes) {
          const response = await prompts({
            ...
            validate: (value) =>
              value && value.trim().length > 0 ? true : "Package specifier cannot be empty.",
          });
