...
export const getEditorHandle = async <E extends PlateEditor = PlateEditor>(
  ...
  if (editableCount === 0) {
    const error = editable
      ? new Error("getEditorHandle: the given locator did not match any element")
      : new Error("getEditorHandle: could not find a [data-slate-editor] on the page");

    throw error;
  }
  if (editableCount > 1) {
    const error = editable
      ? new Error("getEditorHandle: the given locator matched more than one element")
      : new Error(
