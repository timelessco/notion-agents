...
describe("serializeInlineMd", () => {
  ...
  it("preserve leading spaces", () => {
    const nodes = [{ text: "   Hello world" }];
    const result = serializeInlineMd(editor, { value: nodes });
    expect(result).toBe("   Hello world\n");
  });

  it("serialize bold text correctly", () => {
  ...
  it("serialize italic text correctly", () => {
    const nodes = [{ italic: true, text: "Hello" }];
    const result = serializeInlineMd(editor, { value: nodes });
    expect(result).toBe("_Hello_\n");
  });

  it("serialize strikethrough text correctly", () => {
    const nodes = [{ strikethrough: true, text: "Hello" }];
    const result = serializeInlineMd(editor, { value: nodes });
    expect(result).toBe("~~Hello~~\n");
  });

  it("serialize code text correctly", () => {
    const nodes = [{ code: true, text: "Hello" }];
    const result = serializeInlineMd(editor, { value: nodes });
    expect(result).toBe("`Hello`\n");
  });

  it("serialize nested formatting for inlineCode blocks correctly", () => {
    const nodes = [
      { code: true, text: "Code " },
  ...
  it("serialize mixed formatting correctly", () => {
    const nodes = [
      { text: "Hello " },
      { bold: true, text: "bold" },
      { text: " and " },
  ...
  it("serialize nested formatting correctly", () => {
    const nodes = [
      { text: "Hello " },
  ...
  it("serialize overlapping formatting correctly", () => {
    const nodes = [
      { text: "Regular " },
      { bold: true, text: "Bold" },
  ...
  it("serialize overlapping formatting correctly without spaces", () => {
    const nodes = [
      { text: "Regular" },
      { bold: true, text: "Bold" },
      { bold: true, italic: true, text: "Bold&Italic" },
  ...
  it("handle code with other formatting", () => {
    const nodes = [
      { text: "This is " },
  ...
  it("serialize bold empty paragraph as empty paragraph", () => {
