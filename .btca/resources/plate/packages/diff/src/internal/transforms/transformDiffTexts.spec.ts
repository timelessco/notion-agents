...
describe("transformDiffTexts", () => {
  it("throws when either side is empty", () => {
    expect(() => transformDiffTexts([], [{ text: "next" }] as any, options)).toThrow(
      "must have at least one nodes",
    );
    expect(() => transformDiffTexts([{ text: "prev" }] as any, [], options)).toThrow(
      "must have at least one nextNodes",
    );
  });
