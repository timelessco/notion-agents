...
const createOptions = (overrides: Partial<AutoformatTextOptions> = {}): AutoformatTextOptions => ({
  format: "—",
  match: "--",
  mode: "text",
  text: "-",
...
describe("autoformatText", () => {
  it("returns false when the trigger does not match", () => {
    ...
    expect(autoformatText({ selection: { anchor: {} } } as any, createOptions())).toBe(false);
  ...
  it("delegates to a formatter callback when provided", () => {
    ...
    const pointsSpy = spyOn(matchPointsModule, "getMatchPoints").mockReturnValue(matched);
    const del = mock();
    const formatCalls: any[] = [];
    const format: Extract<AutoformatTextOptions["format"], Function> = (currentEditor, options) => {
