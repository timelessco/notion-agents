...
const skipSuggestionDeletesMock = mock((_, node) => JSON.stringify(node.children ?? node));
...
describe("applyAISuggestions utils", () => {
  ...
  it("replaces multi-block chat nodes, updates block selection, and persists replace ids", async () => {
    ...
    deserializeMdMock.mockReturnValue([{ children: [{ text: "next" }], type: "p" }]);
    ...
    expect(setOption).toHaveBeenCalledWith({ key: "aiChat" }, "_replaceIds", ["id-1", "id-2"]);
  });
  ...
  it("inserts fragment suggestions and selects transient text when editing one block", async () => {
    ...
    expect(insertFragment).toHaveBeenCalledWith([{ __transient: true, text: "done" }]);
