...
export function TabbableEffects() {
  ...
  React.useEffect(() => {
    ...
    const handler = (event: KeyboardEvent) => {
       ...
       * options
       */
      const insertedTabbableEntries = insertTabbableEntries?.(event) as TabbableEntry[];
      ...
      if (
        globalEventListener &&
        event.target &&
        ![editorDOMNode, ...insertedTabbableEntries.map(({ domNode }) => domNode)].some(
       ...
       * tab order.
       */
      const tabbableEntries = [...insertedTabbableEntries, ...defaultTabbableEntries].sort((a, b) =>
      ...
      const activeTabbableEntry =
        (activeElement && tabbableEntries.find((entry) => entry.domNode === activeElement)) ?? null;
