...
describe("insertListItem", () => {
  it("returns false without a selection", () => {
    expect(insertListItem({ getType: (key: string) => key } as any)).toBe(false);
  });
  ...
  it("inserts a sibling list item before when the cursor is at the start", () => {
    ...
    const editor = {
      api: {
        ...
        isStart: mock(() => true),
        marks: mock(() => null),
        parent: mock(() => [{ checked: true, children: [{}, {}], type: KEYS.li }, [0]]),
      },
    ...
    expect(insertListItem(editor, { inheritCheckStateOnLineStartBreak: true })).toBe(true);
  ...
  it("inserts a sibling list item after when the cursor is at the end", () => {
    ...
    expect(insertListItem(editor, { inheritCheckStateOnLineEndBreak: true })).toBe(true);
