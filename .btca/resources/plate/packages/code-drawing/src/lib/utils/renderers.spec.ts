...
const { renderCodeDrawing, renderFlowchart, renderGraphviz, renderMermaid, renderPlantUml } =
...
describe("renderPlantUml", () => {
  ...
  it("encodes the diagram, fetches the svg, and returns a data url", async () => {
    ...
    expect(plantUmlEncode).toHaveBeenCalledWith("@startuml\nAlice -> Bob\n@enduml");
...
describe("renderCodeDrawing", () => {
  it("returns an empty string for blank content", async () => {
    await expect(renderCodeDrawing(CODE_DRAWING_TYPE.Mermaid, "   ")).resolves.toBe("");
  });
