...
export const withToggle: OverrideEditor<ToggleConfig> = ({
  ...
  api: {
    isSelectable(element: any) {
      if (isNode(element) && isInClosedToggle(editor, element.id as string)) return false;
  ...
  transforms: {
    deleteBackward(unit: any) {
      if (moveCurrentBlockAfterPreviousSelectable(editor as SlateEditor) === false) return;

      deleteBackward(unit);
    },

    deleteForward(unit: any) {
      if (moveNextSelectableAfterCurrentBlock(editor as SlateEditor) === false) return;
