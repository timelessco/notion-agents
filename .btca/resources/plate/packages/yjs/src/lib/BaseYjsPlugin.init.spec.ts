...
const getSharedChildren = (ydoc: Y.Doc, sharedType?: Y.XmlText | null): unknown[] => {
...
describe("BaseYjsPlugin init", () => {
  ...
  it("turns provider configs into instances, preserves existing providers, and skips connect when autoConnect is false", async () => {
    ...
    const editor = createEditor({
      providers: [{ options: { roomName: "room-1" }, type: providerType }, existingProvider],
    });
  ...
  it("deserializes string values before seeding shared content", async () => {
    ...
    const deserializedValue = [{ type: "p", children: [{ text: "from html" }] }];
    const deserializeSpy = spyOn(editor.api.html, "deserialize").mockReturnValue(
      deserializedValue as any,
    );
    ...
    expect(getSharedChildren(editor.getPlugin(BaseYjsPlugin).options.ydoc)).toEqual(
      deserializedValue,
    );
  });
  ...
  it("awaits async values and falls back to create.value for empty values", async () => {
    ...
    const asyncValue = mock(async () => [{ type: "p", children: [{ text: "async value" }] }]);
    ...
    const createValueSpy = spyOn(fallbackEditor.api.create, "value").mockReturnValue([
      { type: "p", children: [{ text: "fallback" }] },
    ] as any);
    ...
    expect(getSharedChildren(fallbackEditor.getPlugin(BaseYjsPlugin).options.ydoc)).toEqual([
      { type: "p", children: [{ text: "fallback" }] },
    ]);
  });
  ...
  it("uses applyDelta for custom shared types and skips seeding when shared content already exists", async () => {
    ...
    Y.applyUpdate(
      seededDoc,
      await deterministicStateModule.slateToDeterministicYjsState("seeded-doc", [
    ...
    const deterministicSpy = spyOn(deterministicStateModule, "slateToDeterministicYjsState");
  ...
  it("waits for sync before final editor wiring and uses the deterministic update path", async () => {
    ...
    const createdProvider = createMockProvider({ type: "webrtc" });
    const constructorProps: any[] = [];
    const providerType = registerTestProviderType(({ onSyncChange, ...props }) => {
      constructorProps.push(props);
    ...
    const deterministicUpdate = await deterministicStateModule.slateToDeterministicYjsState(
      "sync-doc",
      [{ type: "p", children: [{ text: "hello" }] }] as any,
    );
    ...
    const onReady = mockFn((_: { editor: unknown; isAsync: boolean; value: unknown }) => {
      calls.push("onReady");
    });
