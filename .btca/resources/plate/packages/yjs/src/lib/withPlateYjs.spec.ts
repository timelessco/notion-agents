...
describe("withPlateYjs", () => {
  ...
  it("uses the provided shared type and applies history last", () => {
    ...
    const withTYHistorySpy = spyOn(withTYHistoryModule, "withTYHistory").mockImplementation(((
      editor: any,
    ) => {
  ...
  it("falls back to ydoc content and wires cursors with autoSend disabled", () => {
    ...
    const withTCursorsSpy = spyOn(withTCursorsModule, "withTCursors").mockImplementation(((
      editor: any,
