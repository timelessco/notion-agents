...
describe("imageToBase64", () => {
  ...
  it("rejects invalid urls before fetching", async () => {
    await expect(imageToBase64("javascript:alert(1)")).rejects.toThrow("Invalid URL provided");
  });
  ...
  it("throws when the fetch fails", async () => {
    ...
    await expect(imageToBase64("https://platejs.org/missing.png")).rejects.toThrow(
  ...
  it("converts fetched bytes to base64", async () => {
    ...
    await expect(imageToBase64("https://platejs.org/image.png")).resolves.toBe("SGk=");
  });
});
