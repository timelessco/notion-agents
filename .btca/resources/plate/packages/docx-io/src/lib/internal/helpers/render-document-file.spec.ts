...
describe("render-document-file helpers", () => {
  ...
  it("returns an empty string for null trees and imports paragraphs for text nodes", async () => {
    ...
    const buildParagraphSpy = spyOn(xmlBuilder, "buildParagraph").mockImplementation(async () =>
      fragment({ namespaceAlias: { w: "urn:test" } })
        .ele("@w", "p")
        .up(),
    );

    await expect(convertVTreeToXML(docxDocument, null, nullFragment)).resolves.toBe("");

    const result = await convertVTreeToXML(docxDocument, new VText("hello") as any, xmlFragment);

    expect(buildParagraphSpy).toHaveBeenCalledWith(expect.any(VText), {}, docxDocument);
