...
describe("PathApi", () => {
  ...
  describe("predicates and comparisons", () => {
    ...
    it.each(compareCases)("compares $path with $another", ({ path, another, expected }) => {
  ...
  describe("ancestor and level traversal", () => {
    it("returns ancestors in both directions", () => {
      ...
      expect(PathApi.ancestors([0, 1, 2], { reverse: true })).toEqual([[0, 1], [0], []]);
    });

    it("returns levels in both directions", () => {
      expect(PathApi.levels([0, 1, 2])).toEqual([[], [0], [0, 1], [0, 1, 2]]);
      expect(PathApi.levels([0, 1, 2], { reverse: true })).toEqual([[0, 1, 2], [0, 1], [0], []]);
    });
  });

  describe("move_node transforms", () => {
    it.each(moveNodeCases)("transforms $path with $op", ({ expected, op, path }) => {
