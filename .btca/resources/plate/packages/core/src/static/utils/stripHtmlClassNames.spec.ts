...
describe("stripHtmlClassNames", () => {
  it("keeps only slate classes by default", () => {
    expect(stripHtmlClassNames('<p class="foo slate-p bar slate-bold">Hello</p>', {})).toBe(
      '<p class="slate-p slate-bold">Hello</p>',
    );
  });
  ...
  it("keeps configured class prefixes", () => {
    expect(
      stripHtmlClassNames('<p class="keep-me prose-p slate-p drop-me">Hello</p>', {
