...
import { pipeRenderTextStatic, pluginRenderTextStatic } from "./pluginRenderTextStatic";
...
describe("pluginRenderTextStatic", () => {
  ...
  it("uses component overrides for matching text nodes", () => {
    const CustomText = ({ children }: any) => <mark data-kind="custom">{children}</mark>;
    const editor = createStaticEditor({
      components: {
        comment: CustomText,
      } as any,
  ...
  it("merges plugin text props before delegating to renderText", () => {
    ...
    const renderText = mock(({ attributes, children }) => (
      <span data-class={attributes.className} data-tone={attributes["data-tone"]}>
        {children}
      </span>
    ));
