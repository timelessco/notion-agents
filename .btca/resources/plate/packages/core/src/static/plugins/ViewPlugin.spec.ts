...
describe("ViewPlugin", () => {
  ...
  describe("integration with createStaticEditor", () => {
    ...
    it("handles copy events without throwing", () => {
      ...
      getSelectedDomNodeSpy = spyOn(getSelectedDomNodeModule, "getSelectedDomNode").mockReturnValue(
        undefined,
      );
      isSelectOutsideSpy = spyOn(isSelectOutsideModule, "isSelectOutside").mockReturnValue(false);
  ...
  describe("setFragmentData override", () => {
    ...
    it("handle copy with selection outside editor", () => {
      ...
      isSelectOutsideSpy = spyOn(isSelectOutsideModule, "isSelectOutside").mockReturnValue(true);
    ...
    it("writes slate fragment, html, and plain text payloads on copy", () => {
      ...
      expect(mockData.setData).toHaveBeenCalledWith("text/html", "<p>Alpha</p>");
