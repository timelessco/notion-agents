...
export const useEditableProps = ({
  ...
  const defaultRenderChunk = usePluginOption(ChunkingPlugin, "contentVisibilityAuto")
  ...
  const renderElement = React.useMemo(
    () => pipeRenderElement(editor, storeRenderElement ?? editableProps?.renderElement),
  ...
  const props: EditableProps = useDeepCompareMemo(() => {
    ...
    return _props;
  }, [decorate, editableProps, renderChunk, renderElement, renderLeaf, renderText]);
