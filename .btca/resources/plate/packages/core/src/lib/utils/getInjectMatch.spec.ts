...
describe("getInjectMatch", () => {
  it("respects isElement, isBlock, and isLeaf filters", () => {
    ...
    const elementMatch = getInjectMatch(editor, editor.getPlugin(elementPlugin));
    ...
    expect(elementMatch({ children: [{ text: "leaf" }], type: "p" } as any, [0])).toBe(true);
    expect(blockMatch({ children: [{ text: "leaf" }], type: "a" } as any, [0])).toBe(false);
    expect(blockMatch({ children: [{ text: "leaf" }], type: "p" } as any, [0])).toBe(true);
    expect(leafMatch({ children: [{ text: "leaf" }], type: "p" } as any, [0])).toBe(false);
  ...
  it("respects targetPlugins and excludePlugins", () => {
    ...
    const targetMatch = getInjectMatch(targetEditor, targetEditor.getPlugin(targetPlugin));
    const excludeMatch = getInjectMatch(excludeEditor, excludeEditor.getPlugin(excludePlugin));

    expect(targetMatch({ children: [{ text: "leaf" }], type: "p" } as any, [0])).toBe(true);
    expect(targetMatch({ children: [{ text: "leaf" }], type: "quote" } as any, [0])).toBe(false);
    expect(excludeMatch({ children: [{ text: "leaf" }], type: "quote" } as any, [0])).toBe(false);
    expect(excludeMatch({ children: [{ text: "leaf" }], type: "p" } as any, [0])).toBe(true);
  });
  ...
  it("respects excludeBelowPlugins and maxLevel", () => {
    ...
    expect(match({ children: [{ text: "leaf" }], type: "p" } as any, [0, 0])).toBe(false);
    expect(match({ children: [{ text: "leaf" }], type: "quote" } as any, [0])).toBe(true);
    expect(match({ children: [{ text: "leaf" }], type: "p" } as any, [0, 0, 0])).toBe(false);
  });
});
