...
export type Deserializer<C extends AnyPluginConfig = PluginConfig> = BaseDeserializer & {
  parse?: (
    options: AnyObject & SlatePluginContext<C> & { element: any },
  ) => Partial<Descendant> | undefined | void;
  query?: (options: AnyObject & SlatePluginContext<C> & { element: any }) => boolean;
};
...
export type ExtendEditorApi<C extends AnyPluginConfig = PluginConfig, EA = {}> = (
  ctx: SlatePluginContext<C>,
) => EA &
...
export type ExtendEditorTransforms<C extends AnyPluginConfig = PluginConfig, EA = {}> = (
  ctx: SlatePluginContext<C>,
) => EA &
  Deep2Partial<EditorTransforms & CorePluginTransforms> & {
    [K in keyof InferTransforms<C>]?: InferTransforms<C>[K] extends (...args: any[]) => any
      ? (...args: Parameters<InferTransforms<C>[K]>) => ReturnType<InferTransforms<C>[K]>
...
export type HtmlDeserializer<C extends AnyPluginConfig = PluginConfig> = BaseHtmlDeserializer & {
  /**
  ...
  query?: (options: SlatePluginContext<C> & { element: HTMLElement }) => boolean;
  toNodeProps?: (
    options: SlatePluginContext<C> & { element: HTMLElement },
  ) => Partial<Descendant> | undefined | void;
};

export type HtmlSerializer<C extends AnyPluginConfig = PluginConfig> = BaseSerializer & {
...
export type InjectNodeProps<C extends AnyPluginConfig = PluginConfig> = BaseInjectProps & {
  query?: (
...
export type NodeStaticProps<C extends AnyPluginConfig = PluginConfig> =
  | ((
      props: SlateRenderElementProps<TElement, C> & SlateRenderLeafProps<TText, C>,
    ) => AnyObject | undefined)
  | AnyObject;

/** @deprecated Use {@link RenderStaticNodeWrapper} instead. */
export type NodeStaticWrapperComponent<C extends AnyPluginConfig = PluginConfig> = (
...
export type NodeStaticWrapperComponentReturnType<C extends AnyPluginConfig = PluginConfig> =
  | React.FC<SlateRenderElementProps<TElement, C>>
  | undefined;
...
export type PartialEditorPlugin<C extends AnyPluginConfig = PluginConfig> = Omit<
  Partial<EditorPlugin<C>>,
  "node"
> & {
...
export type Serializer<C extends AnyPluginConfig = PluginConfig> = BaseSerializer & {
  parse?: (options: AnyObject & SlatePluginContext<C> & { node: Descendant }) => any;
  query?: (options: AnyObject & SlatePluginContext<C> & { node: Descendant }) => boolean;
};

/** The `PlatePlugin` interface is a base interface for all plugins. */
export type SlatePlugin<C extends AnyPluginConfig = PluginConfig> = BasePlugin<C> &
  Nullable<{
  ...
  SlatePluginMethods<C> & {
    ...
    shortcuts: Partial<
      Record<
        | (string & {})
        | Exclude<keyof InferApi<C>[C["key"]], keyof InferTransforms<C>[C["key"]]>
...
export type SlatePluginContext<C extends AnyPluginConfig = PluginConfig> = BasePluginContext<C> & {
  editor: SlateEditor;
...
export type SlatePluginMethods<C extends AnyPluginConfig = PluginConfig> = {
  ...
  configurePlugin: <P extends AnySlatePlugin>(
    ...
    config:
      | ((
          ...
          InferSelectors<P>
        >)
      | SlatePluginConfig<any, InferOptions<P>, InferApi<P>, InferTransforms<P>, InferSelectors<P>>,
  ) => SlatePlugin<C>;
  extend: <EO = {}, EA = {}, ET = {}, ES = {}>(
    extendConfig:
      | ((
    ...
    >
  >;
  extendApi: <EA extends Record<string, (...args: any[]) => any> = Record<string, never>>(
  ...
  >;
  extendEditorApi: <
    EA extends Record<string, ((...args: any[]) => any) | Record<string, (...args: any[]) => any>> =
      Record<string, never>,
  >(
    ...
    PluginConfig<
      ...
      InferOptions<C>,
      {
        [K in keyof (EA & InferApi<C>)]: (EA & InferApi<C>)[K] extends (...args: any[]) => any
          ? (EA & InferApi<C>)[K]
  ...
  extendEditorTransforms: <
    ET extends Record<string, ((...args: any[]) => any) | Record<string, (...args: any[]) => any>> =
      Record<string, never>,
  >(
    ...
    PluginConfig<
      ...
      InferApi<C>,
      {
        [K in keyof (ET & InferTransforms<C>)]: (ET & InferTransforms<C>)[K] extends (
          ...args: any[]
        ) => any
  ...
  extendSelectors: <ES extends Record<string, (...args: any[]) => any> = Record<string, never>>(
    extension: (ctx: SlatePluginContext<C>) => ES,
  ) => SlatePlugin<
    PluginConfig<C["key"], InferOptions<C>, InferApi<C>, InferTransforms<C>, ES & InferSelectors<C>>
  >;
  extendTransforms: <ET extends Record<string, (...args: any[]) => any> = Record<string, never>>(
...
export type TransformOptions<C extends AnyPluginConfig = PluginConfig> = BaseTransformOptions &
  SlatePluginContext<C>;
