...
describe("LengthPlugin", () => {
  ...
  describe("when inserting text", () => {
    ...
    it("handle multiple insertions up to maxLength", () => {
      ...
      expect(editor.children).toEqual([{ children: [{ text: "Hello, Wor" }], type: "p" }]);
    });
  });
  ...
  describe("when deleting text", () => {
    it("allow deleting text", () => {
      ...
      expect(editor.children).toEqual([{ children: [{ text: "Hel" }], type: "p" }]);
    });
  });
  ...
  describe("when pasting text", () => {
    it("truncate pasted text that exceeds maxLength", () => {
      editor = createEditorWithLength(10);
      editor.tf.insertFragment([{ children: [{ text: "This is a long pasted text" }], type: "p" }]);

      expect(editor.children).toEqual([{ children: [{ text: "This is a " }], type: "p" }]);
    });
  });
  ...
  describe("when maxLength is not set", () => {
    it("does not limit text input", () => {
      ...
      editor.tf.insertText("This is a very long text that exceeds any reasonable limit");
