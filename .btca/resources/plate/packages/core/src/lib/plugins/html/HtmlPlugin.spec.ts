...
describe("HtmlPlugin", () => {
  ...
  it("deserializes through api.html using the parsed document body", () => {
    ...
    const deserializeSpy = spyOn(editor.api.html, "deserialize").mockReturnValue(fragment);
    ...
    expect((deserializeSpy.mock.calls[0]?.[0]?.element as HTMLElement).tagName).toBe("BODY");
    expect((deserializeSpy.mock.calls[0]?.[0]?.element as HTMLElement).innerHTML).toBe(
      "<p>Hello</p>",
    );
