...
describe("when element is a body", () => {
  it("returns a fragment with the body children", () => {
    ...
    expect(htmlBodyToFragment(createSlateEditor({ plugins: [BaseParagraphPlugin] }), body)).toEqual(
      output,
    );
  });
});
...
describe("when element is not a body", () => {
  ...
  it("returns undefined", () => {
    expect(htmlBodyToFragment(createSlateEditor(), parseHtmlElement("<div>test</div>"))).toEqual(
      output,
    );
  });
});
