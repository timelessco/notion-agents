...
describe("pipeNormalizeInitialValue", () => {
  ...
  describe("when children is passed to createPlateEditor", () => {
    it("normalize the initial value once", () => {
      ...
      expect(result.current).toEqual([{ children: [{ text: "" }], count: 2, type: "p" }]);
    });
  });
  ...
  describe("withPlate", () => {
    describe("children handling", () => {
      it("use provided children", () => {
        const children = [{ children: [{ text: "Test" }], count: 0, type: "p" }];
        const editor = createPlateEditor({
          plugins,
          value: children,
        });
        ...
        expect(result.current).toEqual([{ children: [{ text: "Test" }], count: 2, type: "p" }]);
      });
      ...
      it("use create.value when children is empty", () => {
        ...
        expect(result.current).toEqual([{ children: [{ text: "Factory" }], count: 2, type: "p" }]);
      });
    });
