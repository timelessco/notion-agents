...
describe("resolvePlugins", () => {
  ...
  it("handle nested plugins", () => {
    const pluginKeys = getResolvedKeys([
      createSlatePlugin({
        key: "parent",
        plugins: [createSlatePlugin({ key: "child1" }), createSlatePlugin({ key: "child2" })],
      }),
    ]);
...
describe("applyPluginsToEditor", () => {
  ...
  it("update existing plugins", () => {
    ...
    const plugins = [createSlatePlugin({ key: "a", node: { type: "newType" } })];
...
describe("applyPluginOverrides", () => {
  ...
  it("handle nested overrides", () => {
    ...
    resolvePlugins(editor, [
      createSlatePlugin({
        ...
        plugins: [createSlatePlugin({ key: "child", node: { type: "originalChild" } })],
      }),
    ]);
  ...
  it("override components based on priority only if target plugin has a component", () => {
    ...
    expect(getPlugin(editor, { key: "b" }).render.node).toBe(HighPriorityComponent);
    ...
    expect(getPlugin(editor, { key: "d" }).render.node).toBe(HighPriorityComponent);

    // Highest priority original component is preserved
    expect(getPlugin(editor, { key: "f" }).render.node).toBe(PreservedOriginalComponent);
  });
  ...
  describe("targetPlugins", () => {
    it("correctly apply targetPluginToInject and merge with existing plugins", () => {
      ...
      expect(
        resolvedPlugin.inject!.plugins!.plugin1.parsers?.html?.deserializer!.parse,
      ).toBeDefined();
      ...
      expect(
        resolvedPlugin.inject!.plugins!.plugin2.parsers?.html?.deserializer!.parse,
      ).toBeDefined();
      ...
      expect(
        resolvedPlugin.inject!.plugins!.plugin3.parsers?.html?.deserializer!.parse,
      ).toBeDefined();
    });
  });
  ...
  it("allow overriding core plugins like DebugPlugin", () => {
    ...
    expect(customLogger).toHaveBeenCalledWith("Test message", "TEST", undefined);
  });
...
describe("resolvePlugins with keyless plugins", () => {
  it("does not add a plugin without a key to the editor", () => {
    ...
    expect(editor.meta.pluginList.some((p) => p.key === "keyedPlugin")).toBe(true);
