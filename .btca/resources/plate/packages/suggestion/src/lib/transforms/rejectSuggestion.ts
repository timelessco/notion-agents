...
import { getInlineSuggestionData, getSuggestionKey, getTransientSuggestionKey } from "../utils";

export const rejectSuggestion = (editor: SlateEditor, description: TResolvedSuggestion) => {
  editor.tf.withoutNormalizing(() => {
    ...
    editor.tf.unsetNodes([description.keyId, KEYS.suggestion, getTransientSuggestionKey()], {
      ...
      match: (n) => {
        ...
        if (
          ...
          if (isLineBreak) return suggestionElement.suggestion.id === description.suggestionId;
    ...
    updateNodes.forEach(([node, path]) => {
      ...
      if ("properties" in targetData) {
        ...
        editor.tf.setNodes(Object.fromEntries(addProps.map((key) => [key, true])), {
          at: path,
        });
      }
