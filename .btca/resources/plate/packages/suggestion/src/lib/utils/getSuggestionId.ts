...
export const getSuggestionKeyId = (node: TElement | TText) => {
  const ids: string[] = Object.keys(node).filter((key) => key.startsWith(`${KEYS.suggestion}_`));
...
export const keyId2SuggestionId = (keyId: string) => keyId.replace(`${KEYS.suggestion}_`, "");
