...
describe("diffToSuggestions", () => {
  ...
  it("keeps separate replacement groups distinct when they are not adjacent", () => {
    ...
    const firstRemovedData = getInlineSuggestionData(value[0].children[1] as any)!;
    const firstInsertedData = getInlineSuggestionData(value[0].children[2] as any)!;
    const secondRemovedData = getInlineSuggestionData(value[1].children[1] as any)!;
    const secondInsertedData = getInlineSuggestionData(value[1].children[2] as any)!;
