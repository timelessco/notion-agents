...
describe("insertMedia", () => {
  ...
  it("inserts an image when getUrl resolves and the target type is image", async () => {
    ...
    insertMediaEmbedSpy = spyOn(mediaModule, "insertMediaEmbed").mockImplementation(() => {});
    const editor = {
    ...
    expect(insertImageSpy).toHaveBeenCalledWith(editor, "https://platejs.org/image.png", {
      at: [0],
    });
