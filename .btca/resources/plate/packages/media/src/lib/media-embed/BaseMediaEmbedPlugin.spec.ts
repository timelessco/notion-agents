...
describe("BaseMediaEmbedPlugin", () => {
  it("configures media embeds as void elements with iframe parsing", () => {
    ...
    const blankIframe = new DOMParser().parseFromString("<iframe></iframe>", "text/html").body
    ...
    expect(transformUrl('<iframe src="https://x.test"></iframe>')).toBe("https://x.test");
    expect(parse({ element: iframe, type: KEYS.mediaEmbed } as any)).toEqual({
      type: KEYS.mediaEmbed,
      url: "https://example.com/embed",
    });
    expect(parse({ element: blankIframe, type: KEYS.mediaEmbed } as any)).toBeFalsy();
  });
});
