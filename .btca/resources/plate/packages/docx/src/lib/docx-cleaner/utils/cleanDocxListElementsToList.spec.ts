...
describe("cleanDocxListElementsToList", () => {
  it("normalizes mso-list ignore casing before list cleanup", () => {
    const isDocxListSpy = spyOn(isDocxListModule, "isDocxList").mockReturnValue(false);
    ...
    expect(root.firstElementChild?.getAttribute("style")).toBe("mso-list:Ignore");

    isDocxListSpy.mockRestore();
  });

  it("inserts the converted list before the original list node", () => {
    const isDocxListSpy = spyOn(isDocxListModule, "isDocxList").mockImplementation(
      (element) => element.tagName === "P",
    );
    const docxListToListSpy = spyOn(listModule, "docxListToList").mockReturnValue({
    ...
    expect(root.firstElementChild?.outerHTML).toBe("<ul><li>Converted</li></ul>");
