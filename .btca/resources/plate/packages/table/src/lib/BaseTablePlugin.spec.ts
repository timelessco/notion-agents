...
import { BaseTableCellHeaderPlugin, BaseTableCellPlugin, BaseTablePlugin } from "./BaseTablePlugin";
...
describe("BaseTablePlugin", () => {
  it("parses table cell background styles and attribute props", () => {
    ...
    const tdParse = editor.getPlugin(BaseTableCellPlugin).parsers!.html!.deserializer!.parse!;
    const thParse = editor.getPlugin(BaseTableCellHeaderPlugin).parsers!.html!.deserializer!.parse!;
    const tdProps = editor.getPlugin(BaseTableCellPlugin).node.props as any;
    const thProps = editor.getPlugin(BaseTableCellHeaderPlugin).node.props as any;
  ...
  it("uses cached selection overrides when they are present", () => {
    ...
    expect(editor.getOption(BaseTablePlugin, "isCellSelected", "c1")).toBe(true);
    expect(editor.getOption(BaseTablePlugin, "isSelectingCell")).toBe(true);
    expect(editor.getOption(BaseTablePlugin, "selectedCellIds")).toEqual(["c1"]);
    expect(editor.getOption(BaseTablePlugin, "selectedTableIds")).toEqual(["t1"]);
  });
});
