...
describe("getCellIndices", () => {
  ...
  it("returns cached indices without recomputing", () => {
    ...
    const computeSpy = spyOn(computeCellIndicesModule, "computeCellIndices").mockReturnValue(
      undefined as any,
    );
  ...
  it("warns and falls back when neither cache nor recompute returns indices", () => {
    ...
    spyOn(computeCellIndicesModule, "computeCellIndices").mockReturnValue(undefined as any);
