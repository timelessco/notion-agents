...
describe("copySelectedBlocks", () => {
  ...
  beforeEach(() => {
    ...
    copyToClipboardSpy = spyOn(copyToClipboardModule, "default").mockImplementation(
  ...
  describe("when copying blocks with empty content", () => {
    ...
    it("copy empty blocks but not call setFragmentData for them", () => {
      ...
      editor.getApi = mock().mockReturnValue({
        blockSelection: {
          getNodes: mock().mockReturnValue([
            ...
            [{ id: "block4", children: [{ text: "Last block" }], type: "p" }, [3]],
          ]),
        },
      });
      ...
      const htmlCall = mockDataTransfer.setData.mock.calls.find((call) => call[0] === "text/html");
  ...
  describe("when copying blocks with content", () => {
    ...
    it("copy all selected blocks with content", () => {
      ...
      expect(mockDataTransfer.setData).toHaveBeenCalledWith("text/plain", expect.any(String));
      expect(mockDataTransfer.setData).toHaveBeenCalledWith("text/html", expect.any(String));
      expect(mockDataTransfer.setData).toHaveBeenCalledWith(
        "application/x-slate-fragment",
        expect.any(String),
      );
