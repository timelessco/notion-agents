...
const getSelectedIds = (editor: SlateEditor) =>
  Array.from(editor.getOption(BlockSelectionPlugin, "selectedIds") ?? []).sort();
...
describe("setSelectedIds", () => {
  ...
  it("replaces the selection when explicit ids are provided", () => {
    editor.setOption(BlockSelectionPlugin, "selectedIds", new Set(["existing"]));
  ...
  it("merges added ids and removes removed ids from selectable elements", () => {
    editor.setOption(BlockSelectionPlugin, "selectedIds", new Set(["existing", "row-2"]));
...
describe("addSelectedRow", () => {
  ...
  it("keeps selection state unchanged when the row cannot be found", () => {
    querySelectorSelectableSpy = spyOn(domUtils, "querySelectorSelectable").mockReturnValue(null);
    editor.setOption(BlockSelectionPlugin, "selectedIds", new Set(["existing"]));
