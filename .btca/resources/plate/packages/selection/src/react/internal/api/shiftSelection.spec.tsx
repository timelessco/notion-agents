...
describe("shiftSelection", () => {
  ...
  describe("Flat structure", () => {
    ...
    describe("when anchor is top-most and SHIFT+DOWN", () => {
      it("expand selection downward", () => {
        ...
        expect(Array.from(selectedIds!).sort()).toEqual(["block1", "block2"].sort());
      });
    });

    describe("when anchor is top-most and SHIFT+DOWN again", () => {
      it("expand further to block3", () => {
        editor.setOption(BlockSelectionPlugin, "selectedIds", new Set(["block1", "block2"]));
        editor.setOption(BlockSelectionPlugin, "anchorId", "block1");

        shiftSelection(editor, "down");

        const selectedIds = editor.getOption(BlockSelectionPlugin, "selectedIds");
        expect(Array.from(selectedIds!).sort()).toEqual(["block1", "block2", "block3"].sort());
      });
    });
    ...
    describe("when anchor is bottom-most and SHIFT+UP", () => {
      it("expand selection upward", () => {
        editor.setOption(BlockSelectionPlugin, "selectedIds", new Set(["block2", "block3"]));
  ...
  describe("Nested structure", () => {
    ...
    it("expand down from parent1 to block3 if anchor is parent1 (top-most)", () => {
      ...
      editor.setOption(BlockSelectionPlugin, "selectedIds", new Set(["parent1"]));
    ...
    it("shrink from parent1 if anchor is block3 (not top-most) SHIFT+DOWN", () => {
      ...
      editor.setOption(BlockSelectionPlugin, "selectedIds", new Set(["block3", "parent1"]));
      editor.setOption(BlockSelectionPlugin, "anchorId", "block3");
    ...
    it("expand up from block4 to block3 if anchor is block4 (bottom-most)", () => {
      ...
      editor.setOption(BlockSelectionPlugin, "selectedIds", new Set(["block3", "block4"]));
      editor.setOption(BlockSelectionPlugin, "anchorId", "block4");

      shiftSelection(editor, "up");

      const selectedIds = editor.getOption(BlockSelectionPlugin, "selectedIds");
      expect(Array.from(selectedIds!).sort()).toEqual(["child1", "block3", "block4"].sort());
    });
    ...
    it("shrink from block4 if anchor is parent1 SHIFT+UP", () => {
      ...
      expect(Array.from(selectedIds!).sort()).toEqual(["parent1", "block3"].sort());
    });
  ...
  describe("Complex columns or table-like structure", () => {
    ...
    it("shrink from table1 if anchor=tr1 SHIFT+DOWN", () => {
      ...
      editor.setOption(BlockSelectionPlugin, "selectedIds", new Set(["table1", "tr1"]));
    ...
    it("shrink from tr1 if anchor=tr2 SHIFT+DOWN", () => {
      ...
      editor.setOption(BlockSelectionPlugin, "selectedIds", new Set(["tr1", "tr2"]));
    ...
    it("skip td / p nodes that are not selectable", () => {
      ...
      const _selectedIds = editor.getOption(BlockSelectionPlugin, "selectedIds");
      ...
      editor.setOption(
        BlockSelectionPlugin,
        "isSelectable",
        (node) => node.type === "table" || node.type === "tr" || node.id === "blockZ",
      );
      // Re-run shiftSelection to see if blockZ is included
      shiftSelection(editor, "down");

      const newSelectedIds = editor.getOption(BlockSelectionPlugin, "selectedIds");
      expect(Array.from(newSelectedIds!).sort()).toEqual(["blockZ", "tr2"].sort());
    });
  });
  ...
  describe("Anchor defaults to top-most/bottom-most if not set", () => {
    it("set anchor to top-most for SHIFT+DOWN", () => {
      ...
      editor.setOption(BlockSelectionPlugin, "selectedIds", new Set(["block2"]));
      ...
      const selectedIds = editor.getOption(BlockSelectionPlugin, "selectedIds");
      expect(Array.from(selectedIds!).sort()).toEqual(["block2", "block3"].sort());
