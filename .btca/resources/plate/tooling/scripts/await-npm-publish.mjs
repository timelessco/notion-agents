...
if (!Array.isArray(publishedPackages) || publishedPackages.length === 0) {
  console.log("No published packages reported by changesets. Skipping npm propagation wait.");
  process.exit(0);
}
...
async function isPublished(pkg) {
  try {
    ...
    try {
      ...
      return Array.isArray(parsed) ? parsed.includes(pkg.version) : parsed === pkg.version;
    } catch {
