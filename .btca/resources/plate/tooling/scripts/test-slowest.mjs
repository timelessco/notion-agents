...
if (xml) {
  ...
  for (const match of xml.matchAll(testcasePattern)) {
    const attrs = Object.fromEntries(
      [...match[1].matchAll(attributePattern)].map(([, key, value]) => [key, value]),
    );
  ...
  const sortedFiles = [...fileRows.values()].sort((a, b) => b.milliseconds - a.milliseconds);
  const slowTests = rows.filter((row) => row.milliseconds >= FAST_TEST_SLOW_CASE_THRESHOLD_MS);
  const slowFiles = sortedFiles.filter(
    (row) => row.milliseconds >= FAST_TEST_SLOW_FILE_THRESHOLD_MS,
  );
  const warnTests = rows.filter((row) => row.milliseconds >= FAST_TEST_WARN_CASE_THRESHOLD_MS);
  const warnFiles = sortedFiles.filter(
    (row) => row.milliseconds >= FAST_TEST_WARN_FILE_THRESHOLD_MS,
  );
  const averageMs =
    rows.length === 0 ? 0 : rows.reduce((sum, row) => sum + row.milliseconds, 0) / rows.length;
  const medianMs =
    rows.length === 0
      ? 0
      : [...rows].sort((a, b) => a.milliseconds - b.milliseconds)[Math.floor(rows.length / 2)]
          .milliseconds;
  ...
  console.log(`Average: ${averageMs.toFixed(3)}ms  Median: ${medianMs.toFixed(3)}ms`);
  console.log(
  ...
  if (
    ...
    if (warnFiles.length > 0) {
      ...
      for (const row of warnFiles.filter(
        ...
        console.log(`  - ${row.file} (${row.milliseconds.toFixed(2)}ms across ${row.tests} tests)`);
      }
    }
    ...
    if (warnTests.length > 0) {
      ...
      for (const row of warnTests.filter(
        ...
        console.log(`  - ${location}${prefix}${row.name} (${row.milliseconds.toFixed(2)}ms)`);
      }
    }
  }
  ...
  if (!profileOnly && (slowTests.length > 0 || slowFiles.length > 0)) {
    ...
    if (slowTests.length > 0) {
      ...
      for (const row of slowTests) {
        ...
        console.error(`  - ${location}${prefix}${row.name} (${row.milliseconds.toFixed(2)}ms)`);
      }
    }
