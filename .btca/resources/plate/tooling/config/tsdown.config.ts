...
const INPUT_FILE = fs.existsSync(INPUT_TS_FILE_PATH) ? INPUT_TS_FILE_PATH : INPUT_TSX_FILE_PATH;
...
const fixBrokenDtsChunkImports = (): Plugin => ({
  ...
  writeBundle(options) {
    ...
    const declarationFiles = walkFiles(outDir).filter((filePath) => filePath.endsWith(".d.ts"));
